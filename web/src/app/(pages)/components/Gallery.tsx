...
export default function Gallery({ images, name }: GalleryProps) {
  ...
  useEffect(() => {
    if (thumbnailsRef.current) {
      const btn = thumbnailsRef.current.children[selectedIndex] as HTMLElement;
      if (btn) {
    ...
    <>
      <style jsx global>{`
        ...
        @keyframes slideInRight {
          from { opacity: 0; transform: translateX(20px) scale(1); }
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 w-full">
        ...
        <div
          ref={thumbnailsRef}
          className="order-2 lg:order-1 flex lg:flex-col gap-3 lg:gap-4 overflow-x-auto lg:overflow-y-auto lg:h-[580px] scrollbar-hide lg:w-[80px] shrink-0 no-scrollbar touch-pan-x"
        >
