...
export const cancelOrder = async (req: Request, res: Response) => {
  try {
    ...
    if (orderStatus !== 'placed' && orderStatus !== 'confirmed') {
      return res.status(400).json({
        message: 'Order can only be cancelled when in "placed" or "confirmed" status',
...
// PATCH /api/orders/:id/return/approve (Admin - Approve return)
export const approveReturn = async (req: Request, res: Response) => {
  try {
    ...
    if (orderStatus !== 'return_requested') {
      return res.status(400).json({
        message: 'Order must be in "return_requested" status',
        currentStatus: orderStatus
      });
    }
